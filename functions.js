...
背景动图: {
  ...
  let opacity = 0.3;
  function loadImg(x) {
    let randSrc = imgSources[Math.floor(Math.random() * imgSources.length)];
    let randImg = Math.floor(Math.random() * (randSrc.last - randSrc.first + 1)) + randSrc.first;
    x.src = `https://yys.res.netease.com/pc/zt/20170731172708/data/picture/${randSrc.folder}/${randSrc.subfolder??""}${randImg}/1366x768.${randSrc.format}`;
  }
  loadImg(img1);
...
获取式神资料: {
  (async () => {
    let rawData = await fetch("https://raw.githubusercontent.com/LunaticSatyr/YinYangShi-Onmyoji/main/%E5%BC%8F%E7%A5%9E%E5%BD%95v3");
...
function get式神名单() {
  ...
  bgImg.src = `https://yys.res.netease.com/pc/zt/20161108171335/data/shishen_big_beforeAwake/${随机式神.编号}.png`;
  ...
  for(let option of 式神名单.options) {
    ...
    option.addEventListener("click", (event) => {
      ...
      // if(option !== 式神名单.options[随机位置]){
      //   bgImg.remove();
      //   let newBgImg = document.createElement("img");
      //   newBgImg.setAttribute("class", "background");
      //   newBgImg.src = `https://yys.res.netease.com/pc/zt/20161108171335/data/shishen_big_beforeAwake/${option.dataset.编号}.png`;
      //   newBgImg.alt = `${option.textContent}.png`;
      //   newBgImg.addEventListener("load", () => {
      //     gridCell.style.backgroundColor = "rgba(255, 255, 255, 0.6)";
      //     newBgImg.style.opacity = 1;
      //   });
      //   gridCell.insertAdjacentElement("beforeend", newBgImg);
      // }
      // !!!!!!!!!!!!!!!!!!!!!!!!!!!!
      sleep(1).then(() => {
...
function get式神情报(gridCell, 式神, delay=1, isAwake=0) {
  ...
  式神情报.style.backgroundImage = `URL("https://yys.res.netease.com/pc/zt/20161108171335/data/shishen_big_beforeAwake/${式神.编号}.png")`;
